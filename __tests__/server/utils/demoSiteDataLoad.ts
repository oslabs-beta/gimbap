...
import {EndpointBucketsModel,startWatchingEndpointModel,forceAllPendingUpdates} from './../../../src/server/models/endpointBucketsModel';
...
describe('Populate database with 3 clusters, with 3 methods per cluster, and around 50 endpoints', () => {
  ...
  test('Generate and load data', async () => {
    ...
    const numCallDist: DistributionFunction = () => 500;

    const endpointsPDF: EndpointPDF[] = [];

    for (const method of ['GET', 'POST', 'DELETE']){
      for (const pdf of [pdf1, pdf2, pdf3]) {
        for (let i = 0; i < 2 + Math.ceil(Math.random()*2); i++) {
    ...
    const endpoints: Endpoint[] = simulateServerResponses(endpointsPDF, numCallDist, 1);
